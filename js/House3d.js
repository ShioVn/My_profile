...
// ---------- Minimal CPU-saving start/stop (paste after Zdog init) ----------
let rafId = null;
let running = false;

// simple loop uses your existing update/render functions if present
function loop() {
  // call your existing update/render logic (if you used names update()/render())
  try {
    if (typeof update === 'function') update();      // optional: your update logic
    if (typeof render === 'function') render();      // optional: your render logic
  } catch (e) {
    console.error('Zdog frame error', e);
  }
...
// stop when page hidden, start when visible (if card active)
document.addEventListener('visibilitychange', () => {
  if (document.hidden) stopLoop();
  else {
    // start only if card tab is active
    if (document.querySelector('section.tab.active')?.id === 'card') startLoop();
  }
});

// wrap existing showTab to start/stop based on chosen tab
if (typeof window.showTab === 'function') {
  const _orig = window.showTab;
  window.showTab = function(name) {
    const res = _orig.apply(this, arguments);
    // tiny delay to let showTab set .active
    setTimeout(() => {
      if (name === 'card' || document.querySelector('section.tab.active')?.id === 'card') startLoop();
      else stopLoop();
    }, 10);
    return res;
  };
...
// start initially only if card is active
if (document.querySelector('section.tab.active')?.id === 'card') startLoop();
